...
def dropPage():
  ...
  if 'cols_to_remove' not in ds:
    ds.cols_to_remove = []

  default_cols = [c for c in ds.get("cols_to_remove", []) if c != label]
  ...
  st.multiselect(
    ...
    max_selections=col_count - 1,  # cannot remove all columns
...
def removed_cols_df():
  ...
  return df.drop(columns=ds.cols_to_remove, errors='ignore')
